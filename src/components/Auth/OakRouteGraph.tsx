...
const OakRouteGraph = (props: Props) => {
  ...
  const authenticate = async (type: string, redirect = true) => {
    ...
    const accessToken = props.cookies.get(`aura-access_token`);
    const refreshToken = props.cookies.get(`aura-refresh_token`);
    if (accessToken && refreshToken) {
      ...
      if (data?.authorizeUser) {
        ...
        if (data.authorizeUser.accessToken) {
          ...
          props.cookies.set(`aura-access_token`, newAccessToken);
        }
        dispatch(
          addAuth({
        ...
        props.cookies.remove(`aura-access_token`);
        props.cookies.remove(`aura-refresh_token`);
